"""
SIMSwinTransformerPlus — HCMS 2018 视网膜层分割 GradCAM 可视化
目标层: backbone.fam_layers[i].w_conv (FamLayer 内部的 3×3 ConvModule)

数据集: HCMS 2018 (9类, OCT_Manual_Delineations-2018)
       相较于 Duke 2015 (10类), HCMS 将 OS 和 RPE 分开标注,
       且不含 Fluid (积液) 和第二个背景类.
       图像尺寸: 1024×512, 训练时 crop 至 512×512.
...
# --- 模型配置 (HCMS 2018 训练) ---
CONFIG_PATH = './hcms2018/my_upernet_swin_tiny_patch4_window7_512x512_160k_ade20k_pretrain_224x224_1K.py'
# 模型权重路径
CHECKPOINT_PATH = './hcms2018/best_mDice_epoch_26.pth'

# --- 输入图像 ---
IMAGE_PATH = './hcms2018/example_figure/hc01_spectralis_macula_v1_s1_R_1.png'

# --- 输出配置 ---
OUTPUT_DIR = os.path.join(os.path.dirname(__file__), 'result', 'hcms2018', 'LayerCAM_retina_results_w_conv_hcms2018')
...
torch.backends.cudnn.benchmark = False     # 强制 CUDA 确定性计算
...
# --- 类别配置 (HCMS 2018: 9类, OS 和 RPE 分开, 无 Fluid) ---
CLASS_NAMES = [
    'background',  # 0
    'RNFL',        # 1 - 视网膜神经纤维层
    'GCL+IPL',     # 2 - 节细胞层+内丛状层
...
TARGET_CATEGORIES = [1, 2, 3, 4, 5, 6, 7, 8]  # 排除背景类(0)
...
def main():
    # ---- 打印配置 ----
    print('=' * 70)
    print('  SIMSwinTransformerPlus GradCAM — HCMS 2018 (9类)')
    print('  w_conv 目标层')
    print('=' * 70)
